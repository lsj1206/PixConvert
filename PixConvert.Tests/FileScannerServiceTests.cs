...
public class FileScannerServiceTests : IDisposable
{
    ...
    public FileScannerServiceTests()
    {
        ...
        // 2. 대역 배우(Logger, LanguageService)를 넣어서, 오로지 "FileScannerService"만 순수하게 테스트되도록 환경을 격리합니다.
    ...
    // Task를 반환하는 이유는 FileScannerService의 메서드가 await(비동기) 방식이기 때문입니다.
    [Fact]
    ...
    public async Task AnalyzeSignatureAsync_GivenNonExistentFile_ShouldReturnDashAndLogError()
    {
        ...
        // 이것으로 FileScannerService 로직 안의 "catch 영역"이 정상 작동했음을 수학적으로 입증할 수 있습니다.
        _mockLogger.Verify(
            x => x.Log(
